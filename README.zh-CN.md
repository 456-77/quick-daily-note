...
- **配置随库同步**：插件配置与待办数据保存在库根目录的 `quick-daily-note.json` 单文件中，配合 Remotely Save 等同步工具换设备无需重新设置（文件名特意不带点前缀——Remotely Save 会跳过点开头的文件）；旧版 data.json 与 .quick-daily-note.json 首次加载自动迁移，同步过来的变更自动生效。
